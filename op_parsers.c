...
OperationParserResult parse_arith_constant_op(Parser *parser, OperationParserParams *params) {
    ...
    if (parser_peek(parser, TK_INTEGER)) {
        ...
        if (str_eq(name_str, str_lit("true")) || str_eq(name_str, str_lit("false"))) {
            ...
            if (!result_value) {
                result_value = MLIR_CreateValueOpResult(NULL, 0, bool_type, (string){NULL, 0}, NULL);
            }
    ...
    if (!has_explicit_type && parser_peek(parser, TK_COLON)) {
        ...
        if (parse_type_string(parser, &type_str)) {
            ...
            n_result_types = 1;
            if (!result_value) {
                result_value = MLIR_CreateValueOpResult(NULL, 0, type, (string){NULL, 0}, NULL);
            }
    ...
    if (!result_value) {
        result_value = MLIR_CreateValueOpResult(NULL, 0, NULL, (string){NULL, 0}, NULL);
    }
...
OperationParserResult parse_arith_binary_op(Parser *parser, const OperationParserParams *params) {
        ...
        if (parser_peek(parser, TK_COMMA)) {
            ...
            if (parser_peek(parser, TK_REGISTER)) {
                ...
                if (!operand2) {
                    ...
                    operand2 = MLIR_CreateValueBlockArg(reg_str2, 0, unknown_type, NULL);
                }
    ...
    MLIR_Op *op = MLIR_CreateOp(params->op_type, str_lit(""),
                                      attributes, n_attributes,
                                      result_types, n_result_types,
                                      results, n_results,
        ...
        while (!parser_peek(parser, TK_EOF)) {
            if (parser_peek(parser, TK_LPAREN)) {
                ...
                if (parser_peek(parser, TK_COLON)) {
                    ...
                    MLIR_Value *arg = MLIR_CreateValueBlockArg(reg_str, (uint32_t)func_args.size, arg_type, arg_location);
                    ...
                    if (has_div) {
                        MLIR_Attribute *div_attr = MLIR_CreateAttributeInteger(str_lit("tt.divisibility"), div_value);
                        VecAttribute_push_back(parser->arena, &dict_elems, div_attr);
                    }
                    if (has_max_div) {
                        MLIR_Attribute *max_div_attr = MLIR_CreateAttributeInteger(str_lit("tt.max_divisibility"), max_div_value);
                        VecAttribute_push_back(parser->arena, &dict_elems, max_div_attr);
                    }

                    // Create dictionary attribute (empty if no attributes)
                    MLIR_Attribute *arg_dict = MLIR_CreateAttributeDict(str_lit(""), dict_elems.data, dict_elems.size);
...
OperationParserResult parse_scf_if_op(Parser *parser, const OperationParserParams *params) {
    ...
    if (parser_peek(parser, TK_ARROW)) {
        ...
        if (parser_peek(parser, TK_LPAREN)) {
            parser_expect(parser, TK_LPAREN);
            size_t cap = 2;
            result_types = arena_alloc_array(parser->arena, MLIR_Type*, cap);
            while (!parser_peek(parser, TK_RPAREN) && !parser_peek(parser, TK_EOF)) {
                string type_str = str_lit("");
                if (!parse_type_string(parser, &type_str)) break;
                if (n_result_types >= cap) {
                    cap *= 2;
                    MLIR_Type **expanded = arena_alloc_array(parser->arena, MLIR_Type*, cap);
    ...
    MLIR_Region **regions = arena_alloc_array(parser->arena, MLIR_Region*, n_regions);
...
OperationParserResult parse_scf_for_op(Parser *parser, const OperationParserParams *params) {
    ...
    if (parser_peek(parser, TK_REGISTER)) {
        ...
        loop_var = MLIR_CreateValueBlockArg(loop_var_name, 0, NULL, NULL);
    ...
    if (parser_peek(parser, TK_NAME) && str_eq(parser_token_str(parser), str_lit("iter_args"))) {
        ...
        if (parser_peek(parser, TK_LPAREN)) {
            ...
            while (!parser_peek(parser, TK_RPAREN) && !parser_peek(parser, TK_EOF)) {
                ...
                MLIR_Value *iter_var = MLIR_CreateValueBlockArg(iter_var_name, 0, NULL, NULL);
    ...
    if (loop_var) {
        ...
        MLIR_Value *iv_block_arg = MLIR_CreateValueBlockArg(loop_name, 0, arg_type, NULL);
    ...
    for (size_t i = 0; i < iter_vars.size; i++) {
        ...
        MLIR_Value *iter_block_arg = MLIR_CreateValueBlockArg(iter_name, (uint32_t)(i + 1), arg_type, NULL);
...
OperationParserResult parse_gpu_launch_op(Parser *parser, const OperationParserParams *params) {
        ...
        if (parser_peek(parser, TK_LPAREN)) {
            ...
            while (!parser_peek(parser, TK_RPAREN) && !parser_peek(parser, TK_EOF)) {
                if (parser_peek(parser, TK_REGISTER)) {
                    ...
                    MLIR_Value *arg = MLIR_CreateValueBlockArg(reg_str, (uint32_t)launch_args.size, mlir_type_create_from_string(parser->arena, str_lit("index")), NULL);
    ...
    MLIR_Block *gpu_block = MLIR_CreateBlock();
    ...
    MLIR_Region *gpu_region = MLIR_CreateRegion();
    ...
    if (parser_peek(parser, TK_LPAREN)) {
        ...
        while (!parser_peek(parser, TK_RPAREN) && !parser_peek(parser, TK_EOF)) {
            ...
            if (parser_peek(parser, TK_REGISTER)) {
                ...
                MLIR_Value *arg = MLIR_CreateValueBlockArg(reg, 0, ty, NULL);
...
OperationParserResult parse_affine_for_op(Parser *parser, const OperationParserParams *params) {
    ...
    if (parser_peek(parser, TK_REGISTER)) {
        ...
        ind_var = MLIR_CreateValueBlockArg(iv_name, 0, mlir_type_create_from_string(parser->arena, str_lit("index")), NULL);
    ...
    if (ind_var) {
        ...
        MLIR_Value *iv_block_arg = MLIR_CreateValueBlockArg(block_name, (uint32_t)MLIR_GetBlockNumArgs(block), mlir_type_create_from_string(parser->arena, str_lit("index")), NULL);
        MLIR_AppendBlockArg(block, iv_block_arg);
