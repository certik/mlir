...
MLIR_OpHandle construct_test_module_full(MLIR_Context *ctx) {
    ...
    // Set regions
    MLIR_RegionHandle *module_regions = arena_alloc(arena, sizeof(MLIR_RegionHandle) * (1));
    ...
    MLIR_AttributeHandle *func_attrs = arena_alloc(arena, sizeof(MLIR_AttributeHandle) * (1));
    ...
    MLIR_TypeHandle *const_result_types = arena_alloc(arena, sizeof(MLIR_TypeHandle) * (1));
    const_result_types[0] = i32_type;

    MLIR_AttributeHandle value_attr = MLIR_CreateAttributeInteger(ctx, str_lit("value"), 5);
    MLIR_AttributeHandle *const_attrs = arena_alloc(arena, sizeof(MLIR_AttributeHandle) * (1));
    const_attrs[0] = value_attr;

    MLIR_ValueHandle *const_results = arena_alloc(arena, sizeof(MLIR_ValueHandle) * (1));
    ...
    MLIR_ValueHandle *add_operands = arena_alloc(arena, sizeof(MLIR_ValueHandle) * (2));
    add_operands[0] = MLIR_GetBlockArg(func_block, 0);
    add_operands[1] = MLIR_GetBlockArg(func_block, 1);

    MLIR_TypeHandle *add_result_types = arena_alloc(arena, sizeof(MLIR_TypeHandle) * (1));
    add_result_types[0] = i32_type;

    MLIR_ValueHandle *add_results = arena_alloc(arena, sizeof(MLIR_ValueHandle) * (1));
    ...
    MLIR_ValueHandle *mul_operands = arena_alloc(arena, sizeof(MLIR_ValueHandle) * (2));
    mul_operands[0] = add_result;
    mul_operands[1] = const_result;

    MLIR_TypeHandle *mul_result_types = arena_alloc(arena, sizeof(MLIR_TypeHandle) * (1));
    mul_result_types[0] = i32_type;

    MLIR_ValueHandle *mul_results = arena_alloc(arena, sizeof(MLIR_ValueHandle) * (1));
    ...
    // func.return %2 : i32
    MLIR_ValueHandle *ret_operands = arena_alloc(arena, sizeof(MLIR_ValueHandle) * (1));
    ...
    MLIR_RegionHandle *func_regions = arena_alloc(arena, sizeof(MLIR_RegionHandle) * (1));
...
int app_main(void) {
    ...
    char **argv = arena_alloc(boot_arena, sizeof(char *) * (argc + 1));
    char *argv_buf = arena_alloc(boot_arena, sizeof(char) * (argv_buf_size + 1));
    if (argc > 0) {
